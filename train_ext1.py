"""
Extension 1: Train the CNN on frequency-domain input.

Uses the same architecture, hyperparameters, and 10-fold CV protocol as the
replication, but reads from a frequency-domain data directory. Compares
directly against the raw time-domain baseline.

Usage:
    python train_ext1.py --repr fft
    python train_ext1.py --repr raw     # time-domain baseline for comparison
"""
...
# Reuse the exact training machinery from the replication
from train import (
    set_seed, train_model, evaluate, compute_fold_metrics
)
from config import (
...
def main():
    ...
    parser.add_argument("--repr", choices=["raw", "fft"], required=True)
    ...
    # Load the chosen representation
    if args.repr == "raw":
        data_dir = Path("data/processed")
    else:
        data_dir = Path(f"data/processed_{args.repr}")

    segments = np.load(data_dir / "segments.npy")
    labels = np.load(data_dir / "labels.npy")
    print(f"Loaded {len(labels)} segments, shape {segments.shape}")
    ...
    out_path = RESULTS_DIR / f"ext1_{args.repr}_results.json"
    with open(out_path, "w") as f:
        json.dump(results, f, indent=2)

    print(f"\n{'='*55}")
    print(f"  EXTENSION 1 RESULTS — {args.repr.upper()}")
    print(f"{'='*55}")
